...
const Report1Form: React.FC = () => {
  const classes = useStyles();
  const [value, setValue] = useState(0);
  const [url, setUrl] = useState('/Reports/GetSurplusDrugs');
  ...
  const updateUrl = (newValue: number = 0): void => {
    switch (newValue) {
      case 0:
        setUrl(
          `/Reports/GetSurplusDrugs?fromDate=${state.fromDate
            .convert('gregorian')
            .setLocale('en')
            ...
            state.geoCode
          }&remainExpDaysFromMinDate=${state.remainExpDaysFromMinDate}`
        );
        break;
      case 1:
        setUrl(
          `/Reports/GetFavoriteDrugs?fromDate=${state.fromDate
            .convert('gregorian')
            .setLocale('en')
            .format('YYYY/MM/DD')}&toDate=${state.toDate
            .convert('gregorian')
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12}>
          <Paper square>
            ...
            >
              <Tab label=" داروهای مازاد" />
              <Tab label=" داروهای مورد علاقه داروخانه ها" />
              <Tab label=" داروهای انتخاب شده در تبادل" />
            </Tabs>
          </Paper>
