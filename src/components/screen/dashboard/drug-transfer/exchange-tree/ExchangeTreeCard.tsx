...
const ExchangeTreeCard: React.FC<Props> = (props) => {
  const { date, title, isYou, showBorder } = props
  ...
  const useStyle = makeStyles((theme) =>
    createStyles({
      root: {
        borderLeft: `${ (showBorder !== undefined && showBorder) ? '3px solid #3A2F8B':'1px solid #3A2F8B'}` ,
        borderRight:`${ (showBorder !== undefined && showBorder) ? '3px solid #3A2F8B':'1px solid #3A2F8B'}` ,
      },
      container: {
        height: 40,
      },
      ...
      {!isYou && (
        <>
          <Grid
            ...
            style={{ opacity: `${date ? '1' : '0.3'}` }}
          >
            <Grid container xs={12} direction="row-reverse" className={stepContainer}>
              <Grid item xs={4} direction="row-reverse" className={dateContainerLeft}>
                <span>{date}</span>
              </Grid>
              <Grid item xs={8} direction="row-reverse" className={titleContainer}>
                <span style={{ fontSize: 12 }}>{title}</span>
              </Grid>
            </Grid>
          </Grid>
