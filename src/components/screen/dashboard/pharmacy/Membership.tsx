...
const Membership: React.FC = () => {
  ...
  const { notChecked, accept } = new MembershipRequest();
  ...
  const acceptHandler = async (
    item: MembershipRequestInterface
  ): Promise<any> => {
    toggleIsOpenSaveModalForm();

    const { id, pharmacyID, accepted, pharmacyComment } = item;
  ...
  const isFormValid = (): boolean => {
    return state.pharmacyComment && state.pharmacyComment.trim().length > 0;
  };
  ...
  const submitAccept = async (
    ...
    const { id, accepted, pharmacyComment } = state;
          ...
          <Paper>
            <DataTable
              ...
              stateAction={async (e: any, row: any): Promise<void> =>
                await acceptHandler(row)
              }
              queryKey={MembershipRequestEnum.GET_NOT_CHECKED}
              queryCallback={notChecked}
              urlAddress={UrlAddress.getPharmacyRequests}
              defaultFilter="acceptDate eq null"
              initLoad={false}
            />
